...
public class ImapCopier implements Runnable, Closeable {
    ...
    /**
     * Add folder to filtered list. Folder is this list will be skipped from
     * synchronization.
     *
     * @param folderName
     *            Folder name
    ...
    /**
     * Open a connection to the source Store from where the messages will be
     * copied when <code>copy</code> method will be executed
     *
     * @param storeType
     *            Type of Store (imap, imaps, pop3, pop3s)
     * @param host
     *            Server host name
     * @param user
     *            User
     ...
     * @param url
     *            URL in the format
     *            </code>protocol://user[:password]@server[:port]</code>
     * @throws MessagingException
     *             Messaging Exception
     */
    ...
    /**
     * Open a connection to the target Store where the messages will be copied
     * when <code>copy</code> method will be executed
     *
     * @param storeType
     *            Type of Store (imap, imaps, pop3, pop3s)
     * @param host
     ...
     *             Messaging Exception
     */
    private Store openConnection(String storeType, String host, String user, String password)
            throws MessagingException {
	...
	log.debug("opening " + urlName.getProtocol() + " connection to " + urlName.getHost() + " with "
	        + urlName.getUsername());
	Properties props = new Properties();
	Session session = Session.getDefaultInstance(props);
    ...
    /**
     * Copy the folders and messages defined with the methods
     * <code>openSourceConnection</code> and <code>openTargetConnection</code>
     *
     * @throws MessagingException
     *             Messaging Exception
     */
    ...
    private void copyMessages(Folder sourceFolder, Folder targetFolder, Message[] notCopiedMessages)
            throws MessagingException {
    ...
    private Message[] getNotCopiedMessages(Folder sourceFolder, Folder targetFolder, LocalDate date)
            throws MessagingException {
	...
	openFolderIfNeeded(sourceFolder, Folder.READ_ONLY);
	List<Message> sourceMessages = Arrays
	        .asList(sourceFolder.search(new AndTerm(new SentDateTerm(ComparisonTerm.GE, startDate),
	                new SentDateTerm(ComparisonTerm.LT, endDate))));
	
	if ( !sourceMessages.isEmpty() ) {
	    
	    openFolderIfNeeded(targetFolder, Folder.READ_ONLY);
	    Message[] targetMessages = targetFolder.search(new AndTerm(new SentDateTerm(ComparisonTerm.GE, startDate),
	            new SentDateTerm(ComparisonTerm.LT, endDate)));
